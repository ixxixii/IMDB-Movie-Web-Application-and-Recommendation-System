...
prec=zeros(k,40,3);
hit_rate = zeros(k,40,3);
false_alarm_rate = zeros(k,40,3);
for kkk=1:3
    for i=1:k
        testIdx = (cvFolds == i);
        trainIdx = ~testIdx;
        ...
        [U_b,V_b,numIter,tElapsed,finalResidual]=wnmfrule_regularization(R_binary_train,W_rating_train,K(kkk),0.1,option);
        ...
        %             %top_L_pred = index(1:L);
            ...
            prec(i,L,kkk) = 1.0*hit_all/count;
            hit_rate(i,L,kkk)=1.0*hit_all/like_all;
            false_alarm_rate(i,L,kkk)=1.0*miss_all/dislike_all;
        end
    end
end
%% Plot hit_rate vs false_alarm rate
...
%% Plot hit-rate, false-alarm rate vs L
x=(1:40);
y1=hit_rate;
y2=false_alarm_rate;
figure;
plot(x,y1,'r',x,y2,'b');
title('hit-rate and false-alarm rate VS L');
ylabel('rate');
xlabel('L');
